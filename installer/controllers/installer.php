...
class Installer extends CI_Controller
{
	...
	public function __construct()
	{
		...
		if (is_file($this->languages_directory.'/'.$current_language.'/'.$this->router->fetch_method().'_lang.php'))
		{
	...
	public function step_3()
	{
		...
		if(isset($permissions['files']) AND is_array($permissions['files']) )
		{
	...
	public function complete()
	{
		...
		$data['multi_site_url'] = $data['website_url'] . ($supported_servers[$server_name]['rewrite_support'] === true ? 'admin/sites' : 'index.php/admin/sites');
	...
	public function statistics()
	{
		...
		[
			...
			'webserver_hash' => md5(site_url().$this->session->userdata('http_server').$this->input->server('SERVER_NAME').$this->input->server('SERVER_ADDR').$this->input->server('SERVER_SIGNATURE')),
		...
		$url = 'http://oxygen-cms.com/stats/add';
		$curl = new Curl;
