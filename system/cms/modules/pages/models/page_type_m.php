...
class Page_type_m extends MY_Model
{
    ...
    public function get($id)
    {
        ...
        //dump($pt);die;
        //load the $pt->theme_struct into the body field
    ...
    /**
     * Similar to get_all, however when we list in admin  we dont want the admins to modify the hidden page types
     */
    public function get_all_public()
    {
        //
        $pts = $this->db->where('hidden',0)->get($this->_table)->result();
    ...
    //same as before but nothin that is not creatable
    public function get_all_public_new()
    {
        //
        $pts = $this->db->where('allow_create',1)->where('hidden',0)->get($this->_table)->result();
    ...
    public function insert($input = [], $skip_validation = false)
    {
        ...
        //
        // Is this a hidden page type ?
        // By default its always public (hidden=0)
        // but we see what the user selcts
        //
        $input['hidden'] = (int) (isset($input['hidden']))?$input['hidden']:0;
