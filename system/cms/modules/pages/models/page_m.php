...
class Page_m extends MY_Model
{
	 ...
	 * @return object
	 */
	public function get_by_uri($uri, $is_request = false, $simple_return = false, $admin_preview = false)
	{
		// it's the home page
		if ($uri === null)
		{
			$page = $this->oxycache->model('page_m', 'get_home', array());
		}
		else
		{
			...
			{
				$page = $this->oxycache->model('page_m', 'get_page_simple', array($uri));
		...
		if ( ! $page->layout = $this->oxycache->model('page_type_m', 'get', array($page->type_id)))
		{
			// Some pillock deleted the page layout, use the default and pray to god they didnt delete that too
			$page->layout = $this->oxycache->model('page_type_m', 'get', array(1));
		}
		...
		if($admin_preview)
		{
			$page->entry_id = $page->entry_id_offline;
		}
		...
		if ($page->entry_id and $page->layout->stream_id)
		{
			...
			// Get Streams
			$stream = $this->oxycache->model('streams_m', 'get_stream', array($page->layout->stream_id));
	...
	{
		$page = $this->db
			->select('pages.*, page_types.id as page_type_id, page_types.stream_id')
	 ...
	 * @return array
	 */
	public function get_descendant_ids($id, $id_array = [])
	{
		$id_array[] = $id;
	...
	public function create($input, $stream = false)
	{
		...
		if ( $stream )
		{
			...
			// set the page_ref to the id of the page
			$input['page_ref_id'] = $id;
			...
			else
			{
				...
				// todo-ncc: Should we do some sort of cleanup, and attempt to rebuild ?
				return false;
			}
		}
	...
	public function edit_offline_entry_id($id, $entry_id_offline)
	{
		// validate the data and update
		$result = $this->update($id, array(
			'entry_id_offline'=> (int) $entry_id_offline,
		));
		...
		foreach ($ids as $id)
		{
			...
			if ($page->stream_id)
			{
				if ($stream = $this->streams_m->get_stream($page->stream_id))
				{
					...
					//also delete the offline-fraft copies if they exist
					if($page->entry_id_offline != null)
						$this->streams->entries->delete_entry($page->entry_id_offline, $stream->stream_slug, $stream->stream_namespace);
				}
			}
		}
