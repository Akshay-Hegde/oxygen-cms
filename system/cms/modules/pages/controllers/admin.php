 ...
 * Pages controller
 *
 * @author 		OxygenCSM (2015)
 * @author 		PyroCMS Dev Team (2008-2014)
 *
 *
 * @package 	OxygenCSM\Core\Modules\Pages\Controllers
 */
class Admin extends Admin_Controller {
	...
	public function __construct()
	{
		...
		//see if the layout exist, then try the second option, if not use default
	...
	public function index()
	{
		...
		//
		// Do they have direct specific access to edit this page
		//
		if( (group_has_role('pages', 'manage_pages') == false) 
			AND group_has_role('pages', 'manage_specific_pages') 
			AND ($this->current_user->group !== 'admin'))
		{
			// we need to lock the page ordering
		}
		else
		{
			role_or_die('pages', 'manage_pages');

			//if we get here we can add sorting

			$this->template->append_js('jquery/jquery.ui.nestedSortable.js');
		}
	...
	public function choose_type()
	{
		...
		role_or_die('pages','create_pages');

		// Get our types.
		$this->load->model('page_type_m');

		$page_types = $this->page_type_m->get_all_public_new();
        ...
        {
            redirect('admin/pages/create?page_type='.$page_types[0]->id.$parent);
        }

		$this->template
			//->set_layout(false)
			->set('parent',$parent)
			->set('page_types',$page_types)
			->build('admin/choosetype2');	
	}
	...
	public function ajax_page_details($id)
	{
		...
		if( (group_has_role('pages', 'manage_pages') == false) 
			AND group_has_role('pages', 'manage_specific_pages') 
			AND ($this->current_user) AND $this->page_permissions_m->has_admin_access($this->current_user->id,$id))
		{
			// we need to lock the page ordering
			//give them access to edit this page
			$show_edit = true;
		}
		else
		{
			if(group_has_role('pages', 'manage_pages'))
			{
				$show_edit = true;
			}
		}
		...
		$this->load->view('admin/ajax/page_details', array('allow_edit'=>$show_edit,'page' => $page));
	}
	...
	{
		$page = $this->page_m->get($id);
		$page->preview_uri = $page->uri . '?preview=preview';
		$this->template
			->set_layout('modal') //we dont have the modal anymore
			->build('admin/preview', array('page' => $page ));
	}
	...
	{
		//show_error("Restricted Access-This function is not available in OxygenCMS 1.0");
		//die;
		...
		//https://github.com/pyrocms/pyrocms/commit/88d3964de89894af47ac050a40839862ba6d1593
		...
		//force this value
		$page['entry_id_offline'] = null; 

		// true signifies a duplicate, so when a stream is passed we 
		// can create a new stream based off the stream passed in
		$new_page_id = $this->page_m->create($page, $stream);
		$this->createTimelineEvent($new_page_id,'created',true);
	...
	public function create()
	{
		...
		$this->template
			...
			->set('page', $page)
			->set('stream_fields', $this->streams->fields->get_stream_fields($stream->stream_slug, $stream->stream_namespace, $values,null,null,['page_ref_id']))
	...
	public function edit($id = 0)
	{
		...
		if( group_has_role('pages', 'manage_specific_pages') AND ($this->current_user) AND $this->page_permissions_m->has_admin_access($this->current_user->id,$id))
		{
			// ok they have specific rights
			// this works fine, but the user cant see a 
			// list of pages to eit it in the first place
		...
		// New code for publishing
		if($page->entry_id_offline !=null)
		{
			//however we also need to let the system know this is not to be published
			$page_stream_entry_raw = $this->db->limit(1)->where('id', $page->entry_id_offline)->get($stream->stream_prefix.$stream->stream_slug)->row();
		}
		else
		{
			...
			$the_table = $stream->stream_prefix.$stream->stream_slug;

			$id = $page_stream_entry_raw->id;

			unset($page_stream_entry_raw->id);

			$insert_id = $this->db->insert( $the_table , $page_stream_entry_raw );

			// validate the data and update
			$result = $this->db->where('id',$page->id)->update('pages', array( 'entry_id_offline'=> (int) $this->db->insert_id() ));
		}

		$page_stream_entry_raw->id = $id;
		...
		// Run stream field events
		$this->fields->run_field_events($stream_fields, [], $values);
		...
		$stream_fields = $this->streams->fields->get_stream_fields($stream->stream_slug, $stream->stream_namespace, $values, $page->entry_id,null,['page_ref_id']);	

		$this->_form_data($page->id);

		$this->template
			->title($this->module_details['name'], sprintf(lang('pages:edit_title'), $page->title))
			->set('stream_fields', $stream_fields)
			->set('page', $page)
			->set('parent_page', $parent_page)
			->set('page_type', $page_type)
			->build('admin/form');
	 ...
	 */
	public function push_up($id = 0)
	{
		if($this->push_up_down($id, 'up'))
		{
			//$this->session->set_flashdata('success', sprintf(lang('pages:publish_success'), $id));
			Events::trigger('page_published', $id);
		}
		else
		{
			$this->session->set_flashdata('error', sprintf(lang('pages:publish_fail'), $id));
		}
		redirect('admin/pages/edit/'.$id);
	}

	/**
	 * The opposite of publish 
	 */
	public function pull_down($id = 0)
	{
		if($this->push_up_down($id, 'down'))
		{
			//$this->session->set_flashdata('success', sprintf(lang('pages:pulldown_success'), $id));
		}
		else
		{
			$this->session->set_flashdata('error', sprintf(lang('pages:pulldown_fail'), $id));
		}
		redirect('admin/pages/edit/'.$id);
	}	

	private function push_up_down($id = 0, $direction='up')
	{
		// We are lost without an id. Redirect to the pages index.
		$id or redirect('admin/pages');
		...
		$published_key = ($direction=='up')?'entry_id':'entry_id_offline';
		$unpublished_key = ($direction=='up')?'entry_id_offline':'entry_id';
		...
		$stream = $this->_setup_stream_fields($page_type, 'edit', $page->{$published_key});

		$the_table = $stream->stream_prefix.$stream->stream_slug;

		$live = $this->db->limit(1)->where('id', $page->{$published_key})->get($the_table)->row();
		$draft = $this->db->limit(1)->where('id', $page->{$unpublished_key})->get($the_table)->row();

		//remove the usual suspects
		unset($draft->id);
		unset($draft->created);
		unset($draft->updated);
		unset($draft->created_by);
		unset($draft->ordering_count);

		$draft = (array) $draft;

		if(count($draft)==0) {
			//we dont have any fields in this page type
			return true;
		}
	 ...
	 */
	private function _form_data($page_id=null)
	{
		...
		$this->load->model('pages/page_permissions_m');
		$this->template->system_users = $this->page_permissions_m->get_users_merged($page_id);		
		...
		if ( ! empty($ids))
		{
			foreach ($ids as $id)
			{
				if ($id !== 1)
				{
					...
					$this->comments_m->delete_by_module('pages','page:page',$id);
			...
			if ( ! empty($deleted_ids))
			{
				...
				foreach($deleted_ids as $idx)
					$this->createTimelineEvent($idx,'deleted',false);
	...
	public function createTimelineEvent($id,$method='created',$show_actions=true) {
		...
		if($method=='created') {
			$this->timeline->icon('plus');
		}
		if($method=='deleted') {
			$timeline->SetColor('red')->SetIcon('minus');

		}
		if($show_actions) {
			//$timeline->SetActions( '<a class="btn btn-flat bg-red btn-xs" href="admin/pages/edit/'.$id.'">View Admin</a>' );
		}

		$this->timeline
			->user($this->current_user->id)
			->name('PAGES')
			->message($this->current_user->first_name.' '.$method.' page #'.$id)
			->commit();				
