...
$btn_class ='btn btn-sm btn-flat bg-blue';
$btn_class_delete ='btn btn-sm btn-flat bg-red confirm';
...
<fieldset>
	...
	<p>
		<strong>URL:</strong>
		<a href="<?php echo !empty($page->uri) ? $page->uri : $page->slug ?>" target="_blank">
			<?php echo site_url();?><?php echo !empty($page->uri) ? $page->uri : $page->slug ?>
		</a>
	</p>
	<p>
...
<?php endif ?>
	...
	<?php 
		...
		{
			//echo anchor('admin/pages/choose_type?modal=true&parent='.$page->id, lang('pages:create_label'), "class='{$btn_class} as_modal'");
		}
		else
		{
			$type_id = $this->db->select('id')->limit(1)->get('page_types')->row()->id;
			echo anchor('admin/pages/create?parent='.$page->id.'&page_type='.$type_id, lang('pages:create_label'), "class='{$btn_class}'");
		}

	?>

	<?php if($allow_edit):?>
		<?php echo anchor('admin/pages/duplicate/'.$page->id, lang('pages:duplicate_label'), "class='{$btn_class}'") ?>
		<?php echo anchor('admin/pages/edit/'.$page->id, lang('global:edit'), "class='{$btn_class}'") ?>
	<?php endif;?>

	<?php //echo anchor('admin/pages/preview/'.$page->id, lang('global:preview'), "class='{$btn_class} as_modal'") ?>

	<?php if($allow_edit):?>
		<?php echo anchor('admin/pages/delete/'.$page->id, lang('global:delete'), "class='{$btn_class_delete}'") ?>
	<?php endif;?>

	<!--button type="button" class="btn btn-sm btn-flat btn-default pull-right" data-dismiss="modal">Close</button-->
</div>