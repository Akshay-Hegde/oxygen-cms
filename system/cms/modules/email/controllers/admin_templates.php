...
class Admin_templates extends Admin_Controller
{
	...
	/** @var array */
	private $_edit_default_rules = [];

	/** @var array */
	private $_clone_rules = [];
	...
	public function create()
	{
		...
		$this->template->master_templates = [''] + $this->template->themefile_listing('/templates/email/master/',true);
		$email_template->master_template = '';
		...
		if ($this->form_validation->run())
		{
			if ($email_template->is_default)
			{
				$data = array(
					...
					'master_template' => $this->input->post('master_template'),
				);
			}
			else
			{
	...
	public function preview($id = false)
	{
		...
		$this->template
			->set_layout(false) // was -> set_layout('modal')
			->build('admin/templates/preview', $email_template);
	}
	...
	public function create_copy($id = false)
	{
		...
		if ($this->form_validation->run())
		{
			...
			if ($new_id = $this->email_templates_m->insert($copy))
			{
				...
				redirect('admin/email/templates/edit/'.$new_id);
			}
			else
			{
