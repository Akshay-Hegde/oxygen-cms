...
class Row_m extends MY_Model {
	...
	public $sql = 
	[
		'select'	=> [], 		// will be joined by ','
		'where'		=> [],		// will be joined by 'AND'
		'from'		=> [],		// array of tables
		'join'		=> [],		// array of joins
		'order_by'	=> [],		// will be joined by ','
		'misc'		=> [],		// will be joined by line breaks
	];
	 ...
	 * @return	array
	 */
	public function get_rows($params, $field = null, $stream,$test=false)
	{
		...
		if ( ! in_array('created_by', $disable))
		{
            ...
            $this->sql['select'][] = '`users_profiles`.`display_name` as `created_by||display_name`';
            ...
            $this->sql['join'][] = 'LEFT JOIN '.$this->db->protect_identifiers('users_profiles', true).' as `users_profiles` ON `users_profiles`.`user_id`='.$this->db->protect_identifiers($stream->stream_prefix.$stream->stream_slug.'.created_by', true);
		}
		...
		if (isset($paginate) and $paginate == 'yes')
		{
			...
			if ($cache_query)
			{
				...
				$pag_count = $this->oxycache->get($cache_folder.DIRECTORY_SEPARATOR.$cache_hash.'-count');

				if ($pag_count == false) {
	
					$pag_count = $this->db->query($count_sql)->row()->count;
					$this->oxycache->write($pag_count, $cache_folder.DIRECTORY_SEPARATOR.$cache_hash.'-count', $cache_expires);
				}
			}
			else
			{
		...
		if ($cache_query)
		{
			...
			$rows = $this->oxycache->get($cache_folder.DIRECTORY_SEPARATOR.$cache_hash);

			if ($rows == false)
			{
				$rows = $this->db->query($sql)->result_array();
				$this->oxycache->write($rows, $cache_folder.DIRECTORY_SEPARATOR.$cache_hash, $cache_expires);
			}
		}
		else
		{
	...
	private function process_where($where)
	{
		...
		// check if the tick is only 
		// on one side of the table field call
		// Ticks should only be used for table side of the `.`
		if (strpos($where, '`.') !== false)
		{
			return '('.$where.')';
		}
	...
	public function get_row($id, $stream, $format_output = true, $plugin_call = false)
	{
		...
		// Created By
		$this->db->select($this->db->dbprefix($stream->stream_prefix.$stream->stream_slug).'.*, '.$this->db->dbprefix('users').'.username as created_by_username, '.$this->db->dbprefix('users').'.id as created_by_user_id, '.$this->db->dbprefix('users').'.email as created_by_email');
		$this->db->join('users', 'users.id = '.$this->db->dbprefix($stream->stream_prefix.$stream->stream_slug).'.created_by', 'left');

		$obj = $this->db
						->limit(1)
						->where($this->db->dbprefix($stream->stream_prefix.$stream->stream_slug).'.id', $id)
						->get($this->db->dbprefix($stream->stream_prefix.$stream->stream_slug));
	 ...
	 * sb: we need the params of the plugin so we can extend functionality dynamically
	 *
	 * @access 	public
	 * @params	
	 */
	...
	{		
		...
		$obj = $this->db->query('
			SELECT '.SITE_REF.'_'.STREAMS_TABLE.'.*, '.SITE_REF.'_'.STREAMS_TABLE.'.id as stream_id, '.SITE_REF.'_'.FIELDS_TABLE.'.* 
			FROM '.SITE_REF.'_'.STREAMS_TABLE.', '.SITE_REF.'_'.ASSIGN_TABLE.', '.SITE_REF.'_'.FIELDS_TABLE.'
			WHERE '.SITE_REF.'_'.STREAMS_TABLE.'.id='.SITE_REF.'_'.ASSIGN_TABLE.'.stream_id and
			'.SITE_REF.'_'.FIELDS_TABLE.'.id='.SITE_REF.'_'.ASSIGN_TABLE.'.field_id');
