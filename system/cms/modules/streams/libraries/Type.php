...
class Type
{
    ...
    {    
		...
		if(defined('OXYPATH'))
		{
			define('STREAMS_DIR', OXYPATH.'modules/streams/');
			$types_dir_path = OXYPATH;
		}
		elseif(defined('APPPATH'))
		{
			define('STREAMS_DIR', APPPATH.'modules/streams/');
			$types_dir_path = APPPATH;
		}
		else
		{
			define('STREAMS_DIR', FCPATH.'system/cms/modules/streams/');
			$types_dir_path = STREAMS_DIR.'../../field_types/';
		}		
		...
		$this->addon_paths = 
		[
			//'core' 			=> STREAMS_DIR.'field_types/',
			'core' 			=> $types_dir_path.'field_types/',
			'addon' 		=> ADDONPATH.'field_types/',
			//addon_alt
			'shared' 		=> SHARED_ADDONPATH.'field_types/' 
		];
		...
		{
			Events::trigger('streams_add_addon_path', $this);
		}
	...
	private function _load_type($path, $file, $type, $mode)
	{
		...
		if (class_exists($class_name))
		{
			...
			// $tmp->CI			= get_instance();
			// We already have the reference, no need to fetch 
			// the instance of the reference.
			$tmp->CI			= $this->CI; //get_instance();
	 ...
	 * @param	bool
	 */
	public function load_view($type, $view_name, $data = [] )
	{	
