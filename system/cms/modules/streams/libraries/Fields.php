...
class Fields
{
	...
	public function build_form_input($field, $value = null, $row_id = null, $plugin = false)
	{
		...
		{
			// We only want the form_out_admin ONLY for admin theme, if not exist use default
			if(defined('ADMIN_THEME') AND method_exists($this->CI->type->types->{$field->field_type},'form_output_admin'))
			{
				return $this->CI->type->types->{$field->field_type}->form_output_admin($form_data, $row_id, $field);
			}
			else
			{
				//if the prev is false, then this MUST exist by default rules, if not, its a no no..
     ...
     */
 	public function build_form($stream, $method, $row = false, $plugin = false, $recaptcha = false, $skips = array(), $extra = array(), $defaults = array())
 	{
		...
		[
			...
			'insert_url'				=> null,
			'cancel_url'				=> null,
			'allow_add_another'			=> false, //we make it false for backwards conpat
			'error_start'				=> null,
			'error_end'					=> null,
			'required'					=> '<span>*</span>',
		...
		$this->CI->load->language('streams/streams');
		...
		if ($_POST and $key_check)
		{
			if ($this->CI->form_validation->run() === true)
			{
				...
				if ($extra['return'] or $plugin === true)
				{
					...
					$extra['return'] = str_replace('-id-', $result_id, $extra['return']);
					$extra['insert_url'] = str_replace('-id-', $result_id, $extra['insert_url']);

					//here is where we handle the allow add new redirect
					if(($extra['allow_add_another']===true) )
					{
						if(isset($extra['insert_url']))
						{
							if(($_POST['btnAction']=='save_another') AND ($extra['insert_url']!='')  )
							{
								redirect($extra['insert_url']);
							}
						}
					}
					redirect($extra['return']);
				}
				else
				{
					return $result_id;
	...
	// --------------------------------------------------------------------------
 	...
 	public function entry_request($stream_slug, $namespace_slug, $recaptcha = false, $skips = array(), $extra = array(), $defaults = array())
 	{

 		$this->CI->load->driver('streams');

 		$stream = $this->CI->streams->stream_obj($stream_slug, $namespace_slug);
		...
		[
			...
			'error_end'					=> null,
			'required'					=> '<span>*</span>',
			'success_message'			=> 'lang:streams:new_entry_success',
			'failure_message'			=> 'lang:streams:new_entry_error'
		];
		...
		{
			$this->CI->form_validation->reset_validation();
			$this->set_rules($stream_fields, 'new', $skips, false, $row_id);
		}
		...
		//$values = $this->set_values($stream_fields, false, 'new', $skips, $defaults, $key_check);
	 ...
	 * for new fields
	 */
	public function run_field_setup_events_new_field()
	{
		$this->run_field_setup_events(null,null,null);
	}
