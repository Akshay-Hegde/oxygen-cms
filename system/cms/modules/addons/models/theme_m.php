...
class Theme_m extends MY_Model
{
    ...
    {
        parent::__construct();
        $this->_theme = $this->settings->public_theme;
    ...
    {
		return $this->get_all_by_ref(null);
	}

	/**
	 * This overrides the default ref in a way to scan an additional directory
	 * or site ref.
	 */
    public function get_all_by_ref($other_site_ref=null, $include_core=true)
    {

    	$paths = $this->template->theme_locations();
    	
    	$theme_paths = [];

		foreach($paths as $key=>$path)
		{
			if($include_core===false)
			{
    			if($path==='system/cms/themes/')
    			{
    				continue;
    			}
			}
			...
			//$paths[$key] = $path;
			$theme_paths[$path]=$path;
		}
    	
    	if($other_site_ref !=null && is_string($other_site_ref))
    	{
    		$p = SITE_STORAGE_PATH.$other_site_ref.DIRECTORY_SEPARATOR.'addons'.DIRECTORY_SEPARATOR.'themes'.DIRECTORY_SEPARATOR;
    		$theme_paths[$p] = $p;
    	}

        foreach ($theme_paths as $location)
        {
	...
	public function set_default($input)
	{
		...
		{
			return $this->settings->set('public_theme', $input['theme']);
		}
	...
	private function _spawn_class($location, $slug)
	{
		...
		$details_file = $location.$slug.'/Theme.php';
	 ...
	 * @return array
	 */
	public function get_options_by($params = [])
	{
		return $this->db
			->where($params)
			->get('theme_options')
	 ...
	 * @return array
	 */
	public function get_values_by($params = [])
	{
