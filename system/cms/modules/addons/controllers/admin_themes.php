...
class Admin_themes extends Admin_Controller
{
	...
	public function index()
	{
		...
		foreach ($themes as $theme)
		{
			...
			{
				if ($theme->slug == $this->settings->public_theme)
				{
	...
	public function admin()
	{
		...
		// Render the view
		$this->template
			->append_js('plugins/colorpicker/bootstrap-colorpicker.js')
			->append_css('plugins/colorpicker/bootstrap-colorpicker.css')
			->append_js('plugins/colorpicker/init.js')
			->set('is_admin_themes', true)
			->title($this->module_details['name'])
			->build('admin/themes/index', $data);
	}
	...
	/**
	 * WIP
	 * This will check that all the settings, system-cms modules are all installed
	 * It requires that the Theme.xml file exist. if not it will redirect away.
	 * The Theme.xml will soon replace the Theme.php file which will be more helpful to designers
	 */
	public function check($slug = '')
	{
		...
		$the_theme = $this->theme_m->get($slug);

		/////////////////////////////////////////
		//------    Load the theme xml  -------//
		if(file_exists($the_theme->path.'/Theme.xml'))
		{
			$string = file_get_contents($the_theme->path.'/Theme.xml');

			$xml = simplexml_load_string($string, "\Oxygen\Ui\ThemeDefinition");
			$xml->installWidgetAreas();die;
		}
		/////////////////////////////////////////
	...
	public function options($slug = '')
	{
		...
		if ($this->input->post('btnAction') === 're-index')
		{
			...
			if ($this->theme_m->get_all())
			{
				...
				$this->oxycache->delete_all('theme_m');
		...
		if ($all_options)
		{
			...
			$new_options_array = $options_array;
			...
			if ($this->form_validation->run())
			{
				...
				foreach ($options_array as $option_slug => $stored_value)
				{
					...
					{
						$new_options_array[$option_slug] = $input_value;
		...
		$this->template->append_js('plugins/colorpicker/bootstrap-colorpicker.js');
		$this->template->append_css('plugins/colorpicker/bootstrap-colorpicker.css');
	...
	public function set_default()
	{
		...
		if ($this->theme_m->set_default($this->input->post()))
		{
			...
			//simply calling this loads the namespaces for all the themes.
			$x = $this->theme_m->get($theme);
		...
		if(($this->input->post('method') == 'admin_themes') OR ($this->input->post('method') == 'admin'))
		{
			redirect('admin/addons/admin-themes');
		}
	...
	public function upload()
	{
		...
		{
			$config['upload_path'] = FCPATH.SITE_DIR;
			...
			if ($this->upload->do_upload())
			{
				...
				else
				{
					...
					//lets use an xml or config file for these settings
	...
	public function form_control(&$option)
	{
		...
		switch ($option->type)
		{
			...
			case 'select':
				$form_control = form_dropdown($option->slug, $this->_format_options($option->options), $option->value, 'class="form-control"');
				break;

			case 'select-multiple':
				$options = $this->_format_options($option->options);
				$size = sizeof($options) > 10 ? ' size="10"' : '';
				$form_control = form_multiselect($option->slug.'[]', $options, explode(',', $option->value), 'class="form-control"'.$size);
				break;
			...
			case 'colour-picker':
				$form_control = form_input(array(
					...
					'name' => $option->slug,
					'value' => $option->value,
					'class' => 'form-control colorpicker colorpicker-element'
				));
				break;
		}
