...
class Settings {
	...
	{
		get_instance()->load->model('settings/settings_m');

		get_instance()->lang->load('settings/settings');
	...
	public static function get($name)
	{
		...
		$setting = get_instance()->settings_m->get_by(array('slug' => $name));
	...
	public static function set($name, $value)
	{
		if (is_string($name))
		{
			if (is_scalar($value))
			{
				$setting = get_instance()->settings_m->update($name, array('value' => $value));
			}
	...
	public static function get_all()
	{
		...
		$settings = get_instance()->settings_m->get_many_by(array());
	...
	public static function add($setting)
	{
		...
		return get_instance()->settings_m->insert($setting);
	}
	...
	{
		return get_instance()->settings_m->delete_by(array('slug' => $name));
	}
	...
	public static function form_control(&$setting)
	{
		...
		switch ($setting->type)
		{
			...
			case 'select':
				$_class ='form-control';
				$form_control = form_dropdown($setting->slug, self::_format_options($setting->options), $setting->value, 'class="'.$_class.'"');
				break;
	 ...
	 * @return	array
	 */
	private static function _format_options($options = [])
	{
		$select_array = [];
		...
		foreach ($options as $option)
		{
			...
			if (get_instance()->lang->line('settings:form_option_' . $name) !== false)
			{
				$name = get_instance()->lang->line('settings:form_option_' . $name);
			}
