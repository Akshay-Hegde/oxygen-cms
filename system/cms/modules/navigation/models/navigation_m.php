...
class Navigation_m extends MY_Model
{
	 ...
	 * @return int
	 */
	public function insert_link($input = [])
	{
	 ...
	 * @return bool
	 */
	public function update_link($id = 0, $input = [])
	{
		$input = $this->_format_array($input);
	 ...
	 * @author Jerel Unruh - PyroCMS Dev Team
	 * 
	 * @param  string $group Either the group slug or the group id
	 * @return array An array representing the link tree
	 */
	public function get_link_tree($group, $params = [])
	{
		// the plugin passes the slug
		if ( ! is_numeric($group))
		{
			$row = $this->get_group_by('slug', $group);
		...
		switch($row->link_type)
		{
			...
			case 'page':
				if ($page = $this->page_m->get_by(array_filter(array(
					'id'		=> $row->page_id,
					'status'	=> (is_subclass_of(get_instance(), 'Public_Controller') ? 'live' : null)
				))))
				{
	...
	public function make_url_array($links, $user_group = false, $front_end = false)
	{
		...
		$group = $this->db->select('id')->where('name', $user_group)->get('users_groups')->row();
	...
	{
		return $this->db->order_by('order','asc')->get('navigation_groups')->result();
	}
