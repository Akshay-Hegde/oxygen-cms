...
class Admin extends Admin_Controller {
	...
	[
		...
		[
			...
			'rules'	=> 'trim|required|callback__link_check'
		],
		[
			'field' => 'url',
	...
	public function editx()
	{
		...
		{
			if($new_name = $this->input->post('value'))
			{
				if($id = $this->input->post('pk'))
				{
					$data = new stdClass();
					$data->$field = $new_name;
					$this->db->where('id',$id)->update('navigation_groups',$data);
				}
			}
		}
	}
	 ...
	 * 
	 */
	public function links($group_id=null)
	{
		
		// load model
		$this->load->model('navigation/navgroups_m');

		//get the navigation group
		$navigation = $this->navgroups_m->get($group_id);

		//get the links for the group
		$navigation->links = $this->navigation_m->get_link_tree($group_id);
	...
	public function order()
	{
		...
		if (is_array($order))
		{
			...
			foreach ($order as $i => $link)
			{
				...
				$this->navigation_m->update_by('id', str_replace('link_', '', $link['id']), array('position' => $i, 'parent'=>0));
	...
	public function order_group()
	{
		...
		{
			//reset the order, 
			$this->db->update('navigation_groups',['order'=>0]);

			foreach ($fields as $field_id=>$order) {
				$this->db->where('id',$field_id)->update('navigation_groups',['order'=>$order]);
			}
		}
	}
	 ...
	 * @return
	 */
	public function create($group_id = '',$type ='')
	{
		...
		if ($this->form_validation->run())
		{
			...
			redirect('admin/navigation/links/'.$group_id);
		}
	...
	public function edit($id = 0)
	{
		...
		$this->template
			->set_layout(false)
			->build('admin/ajax/form_'.$link->link_type, compact('link', 'group_options'));
	}
