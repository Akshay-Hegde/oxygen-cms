...
class Plugin_Navigation extends Plugin
{
	...
	public function links()
	{
		...
		$links = $this->oxycache->model('navigation_m', 'get_link_tree', $params, config_item('navigation_cache'));
	...
	{
		$template      = $this->attribute('template','bootstrap-ml');
		$group         = $this->attribute('group');	
		$current_class = $this->attribute('current_class','');	
		...
		$return_string = '';
		$this->load->config('navigation/templates');
		foreach ($links as $link)
		{
			$result = $this->_build_template($template, $link,$current_class);			
			$return_string .= $result['menu'];	
		}		
	...
	private function _build_template($xtemplatex,$link,$current_class,$force_current = false)
	{

		$active_parent = false;


		if ($force_current or preg_match('@^' . current_url() . '/?$@', $link['url']) or ($link['link_type'] == 'page' and $link['is_home']) and site_url() == current_url() )
		{
			$link['current_class'] = $current_class;
			$force_current = true;
		}	
		if($link['children'])
		{

			$template = $this->config->item($xtemplatex.'-parent');

			foreach($link['children'] as $child)
			{
				// is this the link to the page that we're on?
				if ( preg_match('@^' . current_url() . '/?$@', $child['url']) or ($child['link_type'] == 'page' and $child['is_home']) and site_url() == current_url() )
				{
					$link['current_class'] = $current_class;
				}
			...
			foreach($link['children'] as $child)
			{
				$result = $this->_build_template($xtemplatex,$child,$current_class,$force_current);
				$children .= $result['menu'];
				if($result['active_parent'])
				{
					$active_parent = true;
					$link['current_class'] = $current_class;
				}
			...
			$link['list_children'] = $children;

			//finished..
			return ['menu' => $this->parser->parse_string($template, $link, true, true), 'active_parent'=>$active_parent];

		}
		else
		{
			$active_parent = $force_current;


			$template = $this->config->item($xtemplatex.'-child');

			return ['menu' => $this->parser->parse_string($template, $link, true, true), 'active_parent'=>$force_current];
		}
	 ...
	 * @return array|string
	 */
	private function _build_links($links = [], $return_arr = true)
	{
		...
		$child_active_class     = $this->attribute('child_active_class', 'active');
		...
		foreach ($links as $link)
		{
			...
			$item['url']   = $link['url'];
			$item['link_id']   = $link['id']; //userful when {{id is not working becuase of conflict}}
			...
			//$item['attributes']['icon']  = $link['icon'];
			...
			)
			{
				...
				$wrapper['class'][] = $child_active_class;
