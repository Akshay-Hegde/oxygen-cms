...
class Files
{
	...
	{
		get_instance()->load->config('files/files');
		...
		get_instance()->load->model('files/file_m');
		get_instance()->load->model('files/file_folders_m');
		get_instance()->load->spark('cloudmanic-storage/1.0.4');
	}
	...
	public static function create_folder($parent = 0, $name = 'Untitled Folder', $location = 'local', $remote_container = '', $hidden = 0)
	{
		...
		while (get_instance()->file_folders_m->count_by('slug', $slug))
		{
			$i++;
		...
		$id = get_instance()->file_folders_m->insert($insert);
	...
	**/
	public static function create_container($container, $location, $id = 0)
	{
		...
		$result = get_instance()->storage->create_container($container, 'public');

		// if they are also linking a local folder then we save that
		if ($id)
		{
			get_instance()->db->where('id', $id)->update('file_folders', array('remote_container' => $container));
		}
	...
	**/
	public static function folder_contents($parent = 0)
	{
		...
		if ( ! is_numeric($parent))
		{
			...
			$result = get_instance()->file_folders_m->get_by('slug', array_pop($segment));

			$parent = ($result ? $result->id : 0);
		}

		$folders = get_instance()->file_folders_m->where('parent_id', $parent)
			->where('hidden', 0)
			->order_by('sort')
			->get_all();

		$files = get_instance()->file_m->where('folder_id', $parent)
			->order_by('sort')
			->get_all();
		...
		if ($folders)
		{
			foreach ($folders as &$folder) 
			{
				...
				$folder->file_count = get_instance()->file_m->count_by('folder_id', $folder->id);
			}
		}
		...
		if ($files)
		{
			...
			foreach ($files as &$file) 
			{
				...
				$file->keywords = get_instance()->keywords->get_string($file->keywords);
	...
	**/
	...
	public static function folder_tree()
	{
		...
		get_instance()->db->select('id, parent_id, slug, name')->where('hidden', 0)->order_by('sort');
		$all_folders = get_instance()->file_folders_m->get_all();
	...
	**/
	public static function rename_folder($id = 0, $name)
	{
		...
		while (get_instance()->file_folders_m->where('id !=', $id)->count_by('slug', $slug))
		{
			$i++;
			$slug = $original_slug.'-'.$i;
			$name = $original_name.'-'.$i;
		...
		// get the folder to see if it is renamable
		$folder = get_instance()->file_folders_m->get($id);
		if($folder->core==1)
		{
			return self::result(false, lang('files:folder_is_core_rename'), $insert['name']);
		}

		get_instance()->file_folders_m->update($id, $insert);
	...
	**/
	public static function delete_folder($id = 0)
	{
		...
		if ( ! $files = get_instance()->file_m->get_by('folder_id', $id) and ! get_instance()->file_folders_m->get_by('parent_id', $id))
		{
			if($folder->core==1)
			{
				return self::result(false, lang('files:folder_is_core'), $folder->name);
			}

			get_instance()->file_folders_m->delete($id);
	...
	public static function upload($folder_id, $name = false, $field = 'userfile', $width = false, $height = false, $ratio = false, $allowed_types = false, $alt = NULL, $replace_file = false)
	{
		...
		$folder = get_instance()->file_folders_m->get($folder_id);
		...
		if ($folder)
		{
			...
			get_instance()->upload->initialize($upload_config);
			...
			if (get_instance()->upload->do_upload($field))
			{
				...
				$data = array(
					...
					'user_id'		=> (int) get_instance()->current_user->id,
					'type'			=> self::$_type,
				...
				if($replace_file)
				{
					...
					get_instance()->file_m->update($replace_file->id, $data);
				}
				else
				{
					$data['id'] = substr(md5(microtime() . $data['filename']), 0, 15);
					$i = 0;
					while(get_instance()->file_m->exists($data['id']))
					{
					    $data['id'] = substr(md5(microtime() . $data['filename'] . $i++), 0, 15);
					}
					$file_id = $data['id'];
					get_instance()->file_m->insert($data);
				}

				if ($data['type'] !== 'i')
				{
					// so it wasn't an image. Now that we know the id we need to set the path as a download
					get_instance()->file_m->update($file_id, array('path' => '{{ url:site }}files/download/'.$file_id));
				}
	...
	/**
	 * Assign a file to be manged by another module
	 * As all files are assigned to files by default this is the only way to do so
	 */
	public static function alter_permissions($file_id,$module='files',$direct_access=0)
	{
		//prep the changes
		$data = 
		[
		...
		// save its location
		get_instance()->file_m->update($file_id, $data);

		return self::result(true, lang('files:file_status_changed'));
	...
	public static function Default_folder()
	{
		...
		return get_instance()->db->where('slug','default_data_folder')->get('file_folders')->row();
	}
	...
	**/
	...
	{
		$file = get_instance()->file_m->select('files.*, file_folders.name foldername, file_folders.slug, file_folders.location')
		...
		if ($file->location === 'local' and $new_location === 'local')
		{
			...
			if (file_exists(self::$path.$file->filename))
			{
				...
				get_instance()->file_m->update($file_id, array('filename' => $filename, 'name' => $new_name));
		...
		{
			get_instance()->storage->load_driver($new_location);
			...
			if (in_array($container, $containers))
			{
				...
				$path = get_instance()->storage->upload_file($container, self::$path.$file->filename, $object, null, 'public');
				...
				// save its location
				get_instance()->file_m->update($file->id, $data);
		...
		elseif ($file->location and $new_location === 'local')
		{
			...
			get_instance()->load->spark('curl/1.2.1');

			// download the file... dum de dum
			$curl_result = get_instance()->curl->simple_get($file->path);
		...
		elseif ($file->location and $new_location)
		{
			...
			// and we download...
			$curl_result = get_instance()->curl->simple_get($file);
			...
			$path = get_instance()->storage->upload_file($container, $temp_file, $object, null, 'public');
	...
	**/
	public static function get_file($identifier = 0)
	{
		...
		$results = get_instance()->file_m->select('files.*, file_folders.name folder_name, file_folders.slug folder_slug')
	...
	**/
	...
	/**
	 * similar to get_files() but does not check on slug, meaning it will get all files regardless of fodler
	 */
	public static function get_all_files($location = 'local', $container = '')
	{
		$results = get_instance()->file_m->select('files.*, file_folders.name folder_name, file_folders.slug folder_slug')
			->join('file_folders', 'file_folders.id = files.folder_id')
			->where('location', $location)
	...
	**/
	public static function list_files($location = 'local', $container = '')
	{
		...
		{
			$results = get_instance()->file_m->select('filename, filesize')
				->join('file_folders', 'file_folders.id = files.folder_id')
				->where('slug', $container)
				->get_all();
	...
	**/
	...
	{
		$folder = get_instance()->file_folders_m->get_by('id', $folder_id);
		...
		if ($files['status'])
		{
			$valid_records = [];
			$known = [];
			$known_files = get_instance()->file_m->where('folder_id', $folder_id)->get_all();
			...
			foreach ($files['data'] as $file)
			{
				...
				if ( ! array_key_exists($file['filename'], $known))
				{
					$insert = array(
						...
						'folder_id' 	=> $folder_id,
						'user_id'		=> get_instance()->current_user->id,
						'type'			=> $file['type'],
						'name'			=> $file['filename'],
					...
					$valid_records[] = get_instance()->file_m->insert($insert);
				}
				// it's totally not a new file
				else
				{
					// update with the details we got from the cloud
					get_instance()->file_m->update($known[$file['filename']]->id, $file);
			...
			get_instance()->db->where('folder_id', $folder_id)
	...
	**/
	public static function rename_file($id = 0, $name)
	{
		...
		get_instance()->file_m->update($id, array('name' => $name));
	...
	**/
	...
	{
		if ($file_to_replace = get_instance()->file_m->select('files.*, file_folders.name foldername, file_folders.slug, file_folders.location, file_folders.remote_container')
			->join('file_folders', 'files.folder_id = file_folders.id')
	 ...
	 *
	**/
	public static function delete_file($id = 0, $module='files')
	{
		if ($file = get_instance()->file_m->select('files.*, file_folders.name foldername, file_folders.slug, file_folders.location, file_folders.remote_container')
			->join('file_folders', 'files.folder_id = file_folders.id')
			->get_by('files.id', $id))
		{
			get_instance()->load->model('keywords/keyword_m');

			//ok lets just delete regardless
			if($file->direct_access != 1)
			{
				if($file->module != $module)
				{
				 	return self::result(false, 'Cant delete protected file', $file->name);
				}
				//otherwise we are good to go
			}
			

			get_instance()->keyword_m->delete_applied($file->keywords);

			get_instance()->file_m->delete($id);
	...
	/**
	 * Removes all files by module.
	 * Useful for uninstalling a module where files are bound
	 */
	public static function delete_by_module($module='files')
	{
		$files = get_instance()->file_m->where('module',$module)->results();
		...
		foreach($files as $file)
		{
			...
			get_instance()->file_m->delete($file->id);

			self::_unlink_file_by_file_id($file->id);

			$counter++;
		}
		
		return self::result(true, "Removed %s files", $counter);
	...
	**/
	public static function search($search, $limit = 5)
	{
		...
		get_instance()->file_folders_m->select('name, parent_id');
		
		foreach ($search as $match) 
		{
			$match = trim($match);

			get_instance()->file_folders_m->like('name', $match)
				->or_like('location', $match)
				->or_like('remote_container', $match);
		}

		$results['folder'] = get_instance()->file_folders_m->limit($limit)
			->get_all();


		// search the file records
		get_instance()->file_m->select('name, folder_id');

		foreach ($search as $match) 
		{
			$match = trim($match);

			get_instance()->file_m->like('name', $match)
			->or_like('filename', $match)
			->or_like('extension', $match);
		}

		$results['file'] = 	get_instance()->file_m->limit($limit)
			->get_all();

		// search for file by tagged keyword
		$results['tagged'] = get_instance()->file_m->select('files.*')
			->limit($limit)
			->get_tagged($search);
	...
	**/
	public static function allowed_actions()
	{
		...
		foreach (get_instance()->module_m->roles('files') as $value)
		{
			// build a simplified permission list for use in this module
			if (isset(get_instance()->permissions['files']) and array_key_exists($value, get_instance()->permissions['files']) or get_instance()->current_user->group == 'admin')
			{
	...
	**/
	...
	private static function _unlink_file_by_file_id($file_id)
	{
		if ($file = get_instance()->file_m->select('files.*, file_folders.name foldername, file_folders.slug, file_folders.location, file_folders.remote_container')
			->join('file_folders', 'files.folder_id = file_folders.id')
			->get_by('files.id', $file_id))
		{
			return self::_unlink_file($file);
		}
		else
		{
			return false;
