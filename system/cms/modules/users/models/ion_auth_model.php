...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->user_stream 			= $this->streams_m->get_stream('users_profiles', true, 'users');
	...
	public function register($username, $password, $email, $group_id = null, $additional_data = array(), $group_name = false)
	{
		...
		if ($this->streams->entries->insert_entry($additional_data, 'users_profiles', 'users', array(), $extra))
		{
			return $id;
		}
		else
	...
	public function update_user($id, $data, $profile_data)
	{
	    ...
	    // Get the stream
	    $stream = $this->streams_m->get_stream('users_profiles', true, 'users');
	    if ( ! $stream) die('boo');
