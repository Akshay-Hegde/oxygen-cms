...
class Users extends Public_Controller
{
	...
	{
		//set the time backwards to let them    	
    	$six_minutes_ago = strtotime("-6 minutes");
		$this->db->where('id',$this->current_user->id)->update('users',['io_stamp'=>$six_minutes_ago]);
	...
	public function register()
	{
		...
		$profile_validation = $this->streams->streams->validation_array('users_profiles', 'users');
		...
		$assignments = $this->streams->streams->get_assignments('users_profiles', 'users');
		...
		$this->template->set('profile_fields', $this->streams->fields->get_stream_fields('users_profiles', 'users', $profile_data));
	...
	{
		OXYGEN_DEMO and show_error(lang('global:demo_restrictions'));
	...
	public function edit($id = 0)
	{
		...
		$profile_row = $this->db->limit(1)
			->where('user_id', $user->id)->get('users_profiles')->row();
		...
		// Get the profile fields validation array from streams
		$this->load->driver('Streams');
		$profile_validation = $this->streams->streams->validation_array('users_profiles', 'users', 'edit', array(), $profile_row->id);
		...
		$profile_stream_id = $this->streams_m->get_stream_id_from_slug('users_profiles', 'users');
		...
		$this->template->build('profile/edit', array(
			...
			'profile_fields' => $this->streams->fields->get_stream_fields('users_profiles', 'users', $profile_data),
	 ...
	 *
	 * @description
	 * 				This function endpoint will allow a user to change their existing password without the need or fuss
	 *				of updating the entire user profile.
	 * 				It requires only 3 fields old pass/newpass and new pass repeted
	 * @return bool : 
	 */
	public function change_password()
	{

		$this->current_user or redirect('users/register');


		$user = $this->user_m->get(['id' => $this->current_user->id]);	
	

		//only process postback
		if($input = $this->input->post())
		{
			// Clean the fields and remove any unwanted fields
			foreach($input as $key => $field)
			{
				if( ! in_array($key, ['password','password_repeat','password_old']))
				{
					unset($input[$key]);
				}
				else
				{
					//clean
					$input[$key] = escape_tags($field);
				}
			}

			/**
			 * Lets make sure all the keys exist
			 */
			foreach( array('password', 'password_repeat', 'password_old') as $key ) {
			  if( empty($input[ $key ]) ) {
			    $this->session->set_flashdata('error','missing required '.$key.'.');
			    redirect('user/change-password');
			  }
			}


			//lets try to match the password
			...
			if($this->ion_auth->change_password($this->current_user->email, $input['password_old'], $input['password']))
			{
			...
			{
				$this->session->set_flashdata('error','failed to change password. Ensure you enter the existing password correctly.');
				redirect('user/change-password');
			}
		...
		// Render the view
		$this->template->build('profile/change_password', array(
			'_user' => $user,
		));
	}
	...
	public function gravatar($id = 0)
	{
		...
		echo json_encode(['status'=>'success','url' =>$url]);die;
	}
}