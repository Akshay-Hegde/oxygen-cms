...
class Admin extends Admin_Controller
{
	...
	private $validation_rules = array(
		...
		array(
			'field' => 'group_id',
			'label' => 'lang:user:user_group_label',
	...
	public function create()
	{
		...
		$stream_fields = $this->streams_m->get_stream_fields($this->streams_m->get_stream_id_from_slug('users_profiles', 'users'));
		...
		$this->template
			...
			->set('profile_fields', $this->streams->fields->get_stream_fields('users_profiles', 'users', $values))
			->build('admin/form');
	}
	...
	public function edit($id = 0)
	{
		...
		// Get the profile fields validation array from streams
		$this->load->driver('Streams');
		$profile_validation = $this->streams->streams->validation_array('users_profiles', 'users', 'edit', array(), $id);
		...
		if ($id)
		{
			$profile_id = $this->db->limit(1)->select('id')->where('user_id', $id)->get('users_profiles')->row()->id;
		}
		else
		{
			$profile_id = null;
		...
		$profile = $this->db->limit(1)->where('user_id', $id)->get('users_profiles')->row();
		...
		$this->template
			...
			->set('profile_fields', $this->streams->fields->get_stream_fields('users_profiles', 'users', $values, $profile_id))
			->set('member', $member)
			->build('admin/form');
	}
	...
	public function delete($id = 0)
	{
		...
		if ( ! empty($ids))
		{
			...
			foreach ($ids as $id)
			{
				//$member = $this->ion_auth->get_user();

				if ( ! ($member = $this->ion_auth->get_user($id)))
				{
					//cant find the member to delete!
					continue;
				}
				...
				if($this->current_user->group !== 'admin' and (($member->group_id === 1) || $member->group ===  'admin')  )
				{
					$this->session->set_flashdata('error', 'You can not delete an Administrator');
					continue;
				}
