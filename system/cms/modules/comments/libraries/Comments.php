...
class Comments
{
	...
	public function __construct($params)
	{
		...
		get_instance()->load->model('comments/comments_m');
		get_instance()->lang->load('comments/comments');
	...
	public function display()
	{
		...
		$comments = $this->process(get_instance()->comments_m->get_by_entry($this->module, $this->singular, $this->entry_id));
	...
	public function form()
	{
		...
		return $this->load_view('form', array(
			...
			'comment'		=>  get_instance()->session->flashdata('comment'),
		));
	}
	...
	{
		return (int) get_instance()->db->where(array(
			'module'	=> $this->module,
	...
	public function process($comments)
	{
		...
		foreach ($comments as &$comment)
		{
			...
			{
				if (get_instance()->module_m->exists($comment->module))
				{
					get_instance()->lang->load("{$comment->module}/{$comment->module}");
	...
	protected function load_view($view, $data)
	{
		...
		if (file_exists(get_instance()->template->get_views_path().'modules/comments/'.$view.$ext))
		{
			// look in the theme for overloaded views
			$path = get_instance()->template->get_views_path().'modules/comments/';
		}
		else
		{
		...
		get_instance()->load->set_view_path($path);
		get_instance()->load->vars($data);

		return get_instance()->load->_ci_load(array('_ci_view' => $view, '_ci_return' => true));
	}
	...
	{
		return get_instance()->encrypt->encode(serialize(array(
			'id'			=>	$this->entry_id,
