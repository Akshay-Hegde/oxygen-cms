...
class Blog_m extends MY_Model
{
	...
	public function get_all()
	{
		$this->db
			...
			->select('users.username, users_profiles.display_name')
			->join('blog_categories', 'blog.category_id = blog_categories.id', 'left')
			->join('users_profiles', 'users_profiles.user_id = blog.author_id', 'left')
	...
	{
		return $this->db
			->select('blog.*, users.username, users_profiles.display_name')
			->join('users_profiles', 'users_profiles.user_id = blog.author_id', 'left')
			->join('users', 'blog.author_id = users.id', 'left')
			->where('blog.id', $id)
			->get('blog')
	...
	public function get_many_by($params = [])
	{
		...
		if ( ! empty($params['keywords']))
		{
			$this->db
				...
				->or_like('users_profiles.display_name', trim($params['keywords']));
		}
	...
	{
		return $this->db->select('blog.*, blog.title title, blog.slug slug, blog_categories.title category_title, blog_categories.slug category_slug, users_profiles.display_name')
			->from('blog')
			->join('keywords_applied', 'keywords_applied.hash = blog.keywords')
			->join('keywords', 'keywords.id = keywords_applied.keyword_id')
	...
	public function count_by($params = [])
	{
		...
		// we need the display name joined so we can get an accurate count when searching
			->join('users_profiles', 'users_profiles.user_id = blog.author_id');
	...
	public function search($data = [])
	{
		...
		if (array_key_exists('keywords', $data))
		{
			...
			foreach ($phrases as $phrase)
			{
				...
				$this->db->or_like('users_profiles.display_name', $phrase);
				$counter++;
			}
		}
