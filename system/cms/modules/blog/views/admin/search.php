		...
		<tbody>
			<?php foreach ($blog as $post): ?>
				<tr>
					...
					<td>
						<?php echo anchor('admin/blog/preview/' . $post->id, lang($post->status == 'live' ? 'global:view' : 'global:preview'), 'rel="modal-large" class="as_modal  btn btn-flat bg-blue" target="_blank"') . ' | ' ?>
						<?php echo anchor('admin/blog/edit/' . $post->id, lang('global:edit'),['class'=>' btn btn-flat bg-blue']);?> |
						<?php echo anchor('admin/blog/delete/' . $post->id, lang('global:delete'), array('class'=>'confirm btn btn-flat bg-red')) ?>
					</td>
				</tr>
			<?php endforeach ?>
		</tbody>
