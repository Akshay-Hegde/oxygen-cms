...
class Blog extends Public_Controller
{
	...
	public function index()
	{
		...
		$total_entries = count($posts['entries']);
	...
	public function archive($year = null, $month = '01')
	{
		...
		$date = $this->_getDateVariable($month_date);
	...
	/**
	 * Allow the archive view to access variable date formats identifiers for various templating needs
	 */
	private function _getDateVariable($dateObj)
	{
		$a = [];
		$a['month_number'] = $dateObj->format('m');
		$a['month_short'] = $dateObj->format('M');
		$a['year_short'] = $dateObj->format('y');
		$a['year_long'] = $dateObj->format('Y');
		return $a;
	}
	...
	public function tagged($tag = '')
	{
		...
		// Get our comment count whil we're at it.
		// this is a litral copy/duplicate of the code in index().
		// May need to improve the design here.
		$this->row_m->sql['select'][] = "(SELECT COUNT(id) FROM ".
				$this->db->protect_identifiers('comments', true)." WHERE module='blog'
				AND is_active='1' AND entry_key='blog:post' AND entry_plural='blog:posts'
				AND entry_id=".$this->db->protect_identifiers('blog.id', true).") as `comment_count`";
	 ...
	 */
	private function _posts_metadata(&$posts = [])
	{
		$keywords = [];
		$description = [];
	...
	private function _single_view($post)
	{
		...
		if (Settings::get('enable_comments'))
		{
			...
			$post['comment_count'] = $this->comments->count();
			//$this->template->set('comment_count', $this->comments->count());
			//$this->template->set('comments_count', '2');
