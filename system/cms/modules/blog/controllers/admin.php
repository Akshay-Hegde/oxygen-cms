...
class Admin extends Admin_Controller
{
	...
	public function create()
	{
		...
		role_or_die('blog','create_posts');
		
		// They are trying to put this live
		if ($this->input->post('status') == 'live') {
			role_or_die('blog', 'manage_posts');
			$hash = "";
		}
		else {
		...
		if ($this->form_validation->run())
		{
			...
			{
				$this->oxycache->delete_all('blog_m');
				...
				$this->timeline
					...
					->icon('plus')
					->name('Blog')
					->message('Created Blog post '.$extra['title'])
					->commit();	
		...
		if ( ! empty($ids))
		{
			...
			foreach ($ids as $id)
			{
				...
				if ($post = $this->blog_m->get($id))
				{
					...
					{
						$this->comments_m->delete_by_module('module', 'blog')->delete_by('entry_id', null, $id);
