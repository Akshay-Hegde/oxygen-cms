...
class Admin extends Admin_Controller
{
	...
	public function group($group_id)
	{
        ...
        add_template_section($this,'permissions',lang('permissions:back_to_groups'),'admin/permissions',[]);       
		...
		{
			// Set a message to notify the user.
			$this->session->set_flashdata('notice', 'Administrators have access to everything. You can not modify these permissions');
			// Send him to the main index to select a proper group.
			redirect('admin/permissions');
		}
	...
	public function user($user_id)
	{
		...
		$user_ob = $this->user_m->get(['id'=>$user_id]);
		// Get the group data
		$group = $this->group_m->get($user_ob->group_id);
		if ($group->name == 'admin')
		{
			// Set a message to notify the user.
			$this->session->set_flashdata('notice', 'Administrators have access to everything. You can not modify these permissions');
		...
		if ($_POST)
		{
			...
			// Save the permissions.
			if ($this->permission_m->save_user($user_id, $modules, $roles)) {

				// Fire an event. Permissions have been saved.
				Events::trigger('permissions_users_saved', array($user_id, $modules, $roles));
				$this->session->set_flashdata('success', lang('permissions:message_group_saved_success'));
				$this->input->post('btnAction') === 'save_exit' 
					? redirect('admin/permissions')
					: redirect('admin/permissions/user/'.$user_id);					
			}
		}
		...
		// Get the groups permission rules (no need if this is the admin group)
		$edit_permissions = $this->permission_m->get_user($user_id);

		$group_permissions = $this->permission_m->get_group($user_ob->group_id);
		// Get all the possible permission rules from the installed modules
		$permission_modules = $this->module_m->get_all(array('is_backend' => true, 'installed' => true));
		...
		$this->template
			...
			->set('edit_permissions', $edit_permissions)
			->set('group_permissions',$group_permissions)
			->set('group_is_admin', $group_is_admin)
			->set('permission_modules', $permission_modules)
			->set('user', $user_ob)
			->build('admin/user');
