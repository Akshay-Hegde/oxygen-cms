...
class Permission_m extends CI_Model
{
	...
	public function save_user($user_id, $modules, $module_roles)
	{
		// Clear out the old permissions
		$this->db->where('user_id', $user_id)->delete('permissions_users');
		...
		if ($modules)
		{
			...
			foreach ($modules as $module => $permission)
			{
				if ( ! empty($permission))
				{
					...
					// Save this module in the list of "allowed modules"
					if ( ! $result = $this->db->insert('permissions_users', $data))
					{
						// Fail, give up trying
						return false;
					}
	...
	/**
	 * for pages we will have
	 * override_public_acces
	 * override_admin-access
	 */
	/*
	public function save_user_custom($module,$user_id,$role='')
	{
		$row = $this->get($user_id)->row();
		$row->roles = json_decode($row->roles);
		//check if it exist
		if(array_key_exists($role, $row->roles)){
