...
{	
	...
	public function form_output_admin($params, $entry_id, $field)
	{		
		$choices = $this->_choices_to_array($params['custom']['choice_data'], $params['custom']['choice_type'], $field->is_required);
		...
		if ($params['custom']['choice_type'] == 'dropdown')
		{
			...
			return form_dropdown($params['form_slug'], $choices, $value, 'class="form-control" id="'.$params['form_slug'].'"');
		}	
		else
		{
			// -------------------------------
			...
			if($params['custom']['choice_type'] == 'checkboxes' or $params['custom']['choice_type'] == 'multiselect')
			{
				...
				if ( $params['custom']['choice_type'] == 'multiselect' )
				{
					return form_multiselect($params['form_slug'].'[]', $choices, $vals, 'class="form-control" id="'.$params['form_slug'].'"');
				}
			}
	...
	public function param_choice_data($value = null)
	{
		return 
		[
			'input' 		=> form_textarea('choice_data', $value,'class="form-control"'),
	...
	public function param_choice_type($value = null)
	{
		...
		return form_dropdown('choice_type', $choices, $value,'class="form-control"');
	}
