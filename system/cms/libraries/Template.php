...
class Template
{
	...
	private $_override_meta = [];
	private $_override_breadcrumbs = [];
	 ...
	 * @return	string
	 */
	public function build($view, $data = [], $return = false, $IE_cache = true, $pre_parsed_view = false, $template = [])
	{
	...
	public function set_metadata($name, $content, $type = 'meta', $place = 'header', $override = false)
	{
		...
		switch($type)
		{
			...
			case 'link':
				...
				//https://github.com/pyrocms/pyrocms/commit/160b051523d360eb66b57e6a487055a767fcbfb6
	...
	/**
	 * This wrapper first checks the availbility of 
	 * a layout, if it exist it is then set,allow a second preference
	 */
	public function try_layout($layout,$layout2='default')
	{
		if($this->layout_exists($layout)) {
			$this->set_layout($layout);
		}
		else {
			if($this->layout_exists($layout2)) {
				$this->set_layout($layout2);
			}
		}
	}
	...
	public function get_theme_layouts_structs($theme = null)
	{
		$theme OR $theme = $this->_theme;
		...
		foreach ($this->_theme_locations as $location)
		{
			/*
			// Get special web layouts
			if( is_dir($location.$theme.'/views/web/modules/pages/structs/') )
			{
				foreach(glob($location.$theme . '/views/web/modules/pages/structs/*.*') as $layout)
				{
					$layouts[] = pathinfo($layout, PATHINFO_BASENAME);
				}
				break;
			...
			*/
			...
			// So there are no web layouts, assume all layouts are normal layouts
			if(is_dir($location.$theme.'/views/modules/pages/structs/'))
			{
				foreach(glob($location.$theme . '/views/modules/pages/structs/*.*') as $layout)
				{
					$layouts[] = pathinfo($layout, PATHINFO_BASENAME);
				}
				break;
	...
	public function load_struct($struct_filename,& $out_body) 
	{
	...
	/**
	 * Make this a generic function that other functions can use in this class
	 */
	public function themefile_listing($path = '',$public=false) 
	{
		
		$theme  = ($public)?Settings::get('public_theme'):$this->_theme;
		$index = [];
		get_instance()->load->helper('file');
		...
		{
			// So there are no web layouts, assume all layouts are web layouts
			if(is_dir($location.$theme.$path))
			{
				foreach(glob($location.$theme.$path.'*.*') as $afile)
				{
					//$index[$location.$theme.$path] = pathinfo($afile, PATHINFO_FILENAME);
					$index[pathinfo($afile, PATHINFO_FILENAME)] = pathinfo($afile, PATHINFO_FILENAME);		
				}
