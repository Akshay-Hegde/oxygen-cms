...
abstract class Plugin
{
	 ...
	 */
	public function parse_parameter($value, $data = [])
	{
	 ...
	 */
	public function module_view($module, $view, $vars = [], $parse_output = true)
	{
	 ...
	 * @return string The rendered view.
	 */
	public function theme_view($view, $vars = [], $parse_output = true)
	{
	...
	public function locate($plugin, $attributes, $content)
	{
		...
		foreach (array(APPPATH, ADDONPATH, SHARED_ADDONPATH) as $directory)
		{
			...
			else 
			{
				...
				//check if in the theme
				{
					get_instance()->load->model('theme_m');
					$theme = get_instance()->theme_m->get();
					$xpath = $theme->path.'/'.$class.'.php';
					if (file_exists($xpath))
					{
						return $this->_process($xpath, $class, $method, $attributes, $content);
					}
				}				
			}
