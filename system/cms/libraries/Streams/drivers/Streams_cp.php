...
class Streams_cp extends CI_Driver {
	 ...
	 */
	public function entries_table($stream_slug, $namespace_slug, $pagination = null, $pagination_uri = null, $view_override = false, $extra = [])
	{
		$CI = get_instance();
		...
		if ($stream->sorting == 'custom' or (isset($extra['sorting']) and $extra['sorting'] === true))
		{
  		...
  		$data = array(
  			...
  			'search_id'		=> isset($_COOKIE['streams_filters']) ? $_COOKIE['streams_filters'] : null,
  		);
		...
		if(isset($extra['where'])) {
			...
			{
				$where[] = $stream->stream_prefix.$stream->stream_slug.'.'.$_key.' = "'.urldecode($_value).'"';
			}
		...
		if ($view_override)
		{
			...
			$CI->template->build('admin/partials/streams/blank_section', array('content' => $table));
		}
		else
		{
	 ...
	 */
	public function entry_form($stream_slug, $namespace_slug, $mode = 'new', $entry_id = null, $view_override = false, $extra = [], $skips = [], $tabs = false, $hidden = [], $defaults = [])
	{
		$CI = get_instance();
		...
		if ( ! $stream) {
			$this->log_error('invalid_stream', 'form');
			throw new Oxygen\Exceptions\InvalidStreamException();
		}

		// Load up things we'll need for the form
		$CI->load->library(array('form_validation', 'streams/Fields'));
		...
		if ($mode == 'edit')
		{
			if( ! $entry = $CI->row_m->get_row($entry_id, $stream, false))
			{
				...
				throw new Oxygen\Exceptions\InvalidRowException();
			}
		}
		else
		{
		...
		// Set return uri
		if (isset($extra['cancel_url']))
		{
			$data['cancel_url'] = $extra['cancel_url'];
		}

		// but of course opt for cancel_url if it exist
		...
		$CI->template->build('admin/partials/streams/blank_section', $data);
	}
	 ...
	 * see docs for more.
	 */
	public function field_form($stream_slug, $namespace, $method = 'new', $return, $assign_id = null, $include_types = [], $view_override = false, $extra = [], $exclude_types = [], $skips = [])
	{
		$CI = get_instance();
		$data = [];
		...
		if ($CI->input->post('field_type') or $method == 'edit')
		{
			...
			if (isset($CI->type->types->{$field_type}))
			{
				...
				{
					$data['current_field'] = new stdClass();
					$data['current_field']->field_data = [];
				}
				
				// Get our standard params
				require_once(STREAMS_DIR.'libraries/Parameter_fields.php');
	 ...
	 * see docs for more explanation
	 */
	public function fields_table($namespace, $pagination = null, $pagination_uri = null, $view_override = false, $extra = [], $skips = [])
	{
		$CI = get_instance();
	 ...
	 * see docs for more explanation
	 */
	public function assignments_table($stream_slug, $namespace, $pagination = null, $pagination_uri = null, $view_override = false, $extra = [], $skips = [])
	{
		$CI = get_instance();
		$data['buttons'] = (isset($extra['buttons']) and is_array($extra['buttons'])) ? $extra['buttons'] : [];
