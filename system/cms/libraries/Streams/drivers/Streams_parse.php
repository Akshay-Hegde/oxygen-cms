...
class Streams_parse extends CI_Driver {
	...
	public function parse_tag_content($content, $data, $stream_slug, $stream_namespace, $loop = false, $fields = null, $id_name = 'id')
	{
		...
		//$content = str_replace($rep, '{{ streams:multiple stream="'.$stream_slug.'" base_namespace="'.$stream_namespace.'" entry=id ', $content);
		...
		if ($fields)
		{
			foreach ($fields as $field)
			{
				...
				{
					$content = preg_replace('/\{\{\s?'.$field->field_slug.'\s?/', '{{ streams:field row_id="{{ '.
						$id_name.' }}" stream_slug="'.
						$stream_slug.'" field_slug="'.$field->field_slug.'" namespace="'.
						$stream_namespace.'" field_type="'.$field->field_type.'" ', $content);

					$content = preg_replace('/\{\{\s?\/'.$field->field_slug.'\s?\}\}/', '{{ /streams:field }}', $content);
				}
			}
		}
